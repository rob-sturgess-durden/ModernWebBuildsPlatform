#!/usr/bin/env python3
"""
Enhanced Restaurant Homepage Generator
Creates modern, responsive HTML homepages for restaurants from CSV data with real Instagram integration.
"""
...
try:
    from instagram_api_helper import get_instagram_posts_for_handle, generate_instagram_widget_js
except ImportError:
    # Fallback if instagram_api_helper is not available
    def get_instagram_posts_for_handle(handle, limit=5):
        """Fallback function that returns mock data if Instagram API is not available."""
        return []
    
    def generate_instagram_widget_js(handle, posts):
        """Fallback function that generates basic Instagram widget JavaScript."""
        return f"""
        <script>
        // Fallback Instagram Widget for @{handle}
        document.addEventListener('DOMContentLoaded', function() {{
            const grid = document.getElementById('instagram-grid-{handle.replace('.', '-')}');
            if (grid) {{
                grid.innerHTML = `
                    <div class="instagram-placeholder">
                        <i class="fab fa-instagram"></i>
                        <p>Instagram integration not available. Please visit <a href="https://instagram.com/{handle}" target="_blank">@{handle}</a></p>
                    </div>
                `;
            }}
        }});
...
def get_coordinates_for_address(address):
    """Get coordinates for restaurant addresses in Hackney."""
    # Map of addresses to their coordinates (lat, lng)
    address_coordinates = {
    ...
    return address_coordinates.get(address, (51.5454, -0.0556))  # Default to Hackney center
...
def generate_instagram_widget(instagram_handle):
    """Generate Instagram widget HTML with real data integration."""
    if not instagram_handle:
        return ""
    
    # Fetch Instagram posts (real or mock data)
    posts = get_instagram_posts_for_handle(instagram_handle, limit=5)
    
    # Generate JavaScript with real data
    widget_js = generate_instagram_widget_js(instagram_handle, posts)
...
<body>
        ...
        <section id="about" class="section">
            ...
            <div class="about-grid">
                ...
                <div class="about-card">
                    ...
                    <h3>Location</h3>
                    <div class="map-container" id="map-{filename}">
    ...
    <script>
        // Initialize OpenStreetMap for {restaurant_name}
        document.addEventListener('DOMContentLoaded', function() {{
            const mapContainer = document.getElementById('map-{filename}');
            if (mapContainer) {{
                const map = L.map('map-{filename}').setView([{lat}, {lng}], 16);
                ...
                marker.bindPopup('<b>{restaurant_name}</b><br>{address}');
                
                // Add a popup with restaurant info
...
def main():
    """Main function to process CSV and generate homepages with enhanced Instagram integration."""
    csv_file = 'HakcneyResturants.csv'
    output_dir = '../demo-pages'
    
    # Check for Instagram API key
    instagram_token = os.getenv('INSTAGRAM_ACCESS_TOKEN')
    if instagram_token:
        print("✅ Instagram API key found in .env file")
        print("📸 Will attempt to fetch real Instagram posts")
    else:
        print("⚠️  No Instagram API key found in .env file")
        print("📸 Will attempt web scraping for real posts, fallback to mock data")
    ...
    print(f"Generating enhanced homepages in '{output_dir}' directory...\n")
    
    # Track real vs mock posts
    real_posts_count = 0
    mock_posts_count = 0
    ...
    for i, restaurant in enumerate(restaurants, 1):
        try:
            ...
            if instagram:
                # Check if we got real posts by looking at the generated content
                if 'Successfully scraped' in html_content or 'real posts' in html_content.lower():
                    print(f"   📸 Instagram: @{instagram} (with REAL posts)")
                    real_posts_count += 1
                else:
                    print(f"   📸 Instagram: @{instagram} (with themed mock posts)")
                    mock_posts_count += 1
            if facebook:
    ...
    print(f"🎉 Successfully generated {len(restaurants)} enhanced restaurant homepages!")
    print(f"📂 Check the '{output_dir}' folder for all HTML files.")
    
    # Summary of Instagram integration
    if real_posts_count > 0:
        print(f"\n📸 Instagram Integration Summary:")
        print(f"   ✅ {real_posts_count} restaurants with real Instagram posts")
        if mock_posts_count > 0:
            print(f"   🎭 {mock_posts_count} restaurants with themed mock posts")
    elif mock_posts_count > 0:
        print(f"\n📸 Instagram Integration Summary:")
        print(f"   🎭 {mock_posts_count} restaurants with themed mock posts")
        print(f"   ℹ️  Real posts not available due to Instagram's anti-scraping measures")
    ...
    if instagram_token:
        ...
        print("📸 Attempting to fetch real posts from restaurant accounts")
    else:
        print("\n📝 To improve Instagram integration:")
        print("1. Create a .env file in the scripts folder")
        print("2. Add: INSTAGRAM_ACCESS_TOKEN=your_token_here")
        print("3. Get your token from Instagram Basic Display API")
        print("4. Re-run this script for better integration")
